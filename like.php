...
$sql = "SELECT * from like_info where user_id='$user_id'  and status_id='$status_id2'";
$res = mysql_query($sql);

if(mysql_num_rows($res) > 0) {
	$response = array(
			'status'	=>	'error',
			'msg'		=>	'You have alredy liked this'
		);
	echo json_encode($response);
}
else{
	$num=1;
	$sql = "INSERT into like_info (user_id,status_id,info) values ('$user_id','$status_id2','$num')";
	mysql_query($sql);
	$response = array(
			'status'	=>	'success',
			'msg'		=>	''
